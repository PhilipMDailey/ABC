...
public class PlayerBasicAttack : PlayerAttackBase
{
    ...
    void DetectHits(Vector3 position, ICombatant attacker, HashSet<ICombatant> alreadyHit)
    {
        ...
        Debug.Log($"DetectHits called at {position}, sphereSize: {sphereSize}, targetLayers: {targetLayers.value}, hits found: {hits.Length}");
        
        foreach (var hit in hits)
        {
            Debug.Log($"Hit collider: {hit.gameObject.name}, Layer: {hit.gameObject.layer}");
            
            ICombatant target = hit.GetComponent<ICombatant>();
            if (target == null) { Debug.Log($"{hit.gameObject.name} has no ICombatant component."); continue; }
            if (target == attacker) { Debug.Log($"{hit.gameObject.name} is the attacker, skipping."); continue; }
            if (!target.IsAlive) { Debug.Log($"{hit.gameObject.name} is not alive, skipping."); continue; }
            if (alreadyHit.Contains(target)) { Debug.Log($"{hit.gameObject.name} already hit, skipping."); continue; }
    ...
    GameObject CreateSphere()
    {
        ...
        {
            Debug.LogWarning("PlayerBasicAttack: No attackIndicatorMaterial assigned. Assign a transparent URP/Unlit material in the Inspector.");
        }
